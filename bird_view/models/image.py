...
class ImageAgent(Agent):
    ...
    def run_step(self, observations, teaching=False):
        ...
        _rgb = self.attack(_rgb, _speed, _command) 
        with torch.no_grad():
            if self.model.all_branch:
                model_pred, model_preds = self.model(_rgb, _speed, _command)
            else:
        ...
        if self.attacker is not None:
            self.debug['adv_rgb'] = 255. * _rgb.squeeze(0).cpu().numpy().transpose(1, 2, 0)
